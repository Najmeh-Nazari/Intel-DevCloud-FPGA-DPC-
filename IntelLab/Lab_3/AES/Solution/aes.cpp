...
void KeyExpansion(unsigned char inputKey[16], unsigned char expandedKeys[176]) {
    ...
    sycl::range<1> r1{176};
    sycl::range<1> r2{16};
    sycl::buffer<unsigned char,1> mybufA(expandedKeys, r1);
    sycl::buffer<unsigned char,1> mybufB(inputKey, r2);
    
    auto e = myq.submit([&](sycl::handler& h)
      {
          auto expandedKeys_A = mybufA.get_access<sycl::access::mode::write>(h);
          auto inputKey_A = mybufB.get_access<sycl::access::mode::read>(h);

          h.single_task<class theKernel>([=]()
          {
            for (int i = 0; i < 16; i++) {
                expandedKeys_A[i] = inputKey_A[i];
            }
          });
      });
      e.wait();
